...
def menu(hauteur=500, largeur=700):
    """
    ...
    rectangle(largeur // 3, 6 * hauteur // 10, 2 * largeur // 3,
              7 * hauteur // 10, remplissage="blue", tag="menu")
    rectangle(largeur // 3, 8 * hauteur // 10, 2 * largeur // 3,
              9 * hauteur // 10, remplissage="blue", tag="menu")
    texte(largeur // 2, 2.5 * hauteur // 10, "STAY ALIVE !", ancrage='c', couleur="blue",
    ...
    while True:
        ...
        if tev == 'Quitte':
            break
        if tev == "ClicGauche":
            if largeur // 3 < abscisse(evv) < 2 * largeur // 3 and 4 * hauteur // 10 < ordonnee(evv) < 5 * hauteur // 10:
                efface_tout()
                texte(largeur // 2, 2.5 * hauteur // 10, "JEU", ancrage='c', couleur="blue",
                taille=largeur // 18, tag="menu")
            if largeur // 3 < abscisse(evv) < 2 * largeur // 3 and 6 * hauteur // 10 < ordonnee(evv) < 7 * hauteur // 10:
                efface("menu")
                texte(largeur // 2, 2.5 * hauteur // 10, "REGLES", ancrage='c', couleur="blue",
                      taille=largeur // 18, tag="menu")
            if largeur // 3 < abscisse(evv) < 2 * largeur // 3 and 8 * hauteur // 10 < ordonnee(evv) < 9 * hauteur // 10:
                break
